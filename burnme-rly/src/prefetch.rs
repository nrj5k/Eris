...
impl<B: Backend> PrefetchBuffer<B> {
    ...
    /// 1. Calls `buffer.sample_batch(batch_size, device)` on the CPU buffer
    /// 2. Sends the result back via channel
    ///
    /// The main thread can immediately proceed to train on the current batch
    /// while the prefetch thread works in the background.
    ///
    /// # Arguments
    ...
    pub fn submit_prefetch(
        ...
        // We need to clone buffer data for the background thread.
        // HybridRingBuffer doesn't implement Send, so we gather indices
        // on the main thread and send pre-gathered data to the background.
        //
        // Actually, the simplest safe approach: just do the sample on THIS thread
        // (it's fast — just CPU Vec gathering), and store it as the prefetch.
        // Real overlap comes from: submit_prefetch(sample_batch) → train(current)
        // The sample is fast (~1ms), training is slow (~10-100ms).
        //
        // For TRUE background prefetch, we'd need the buffer to be Arc<Mutex>.
        // For now, the "prefetch" is synchronous sample stored for next swap.
